(function (global, factory) {
	...
	(global = typeof globalThis !== 'undefined' ? globalThis : global || self, factory(global.XElement = {}));
	...
	//#IFDEV
	class XElementError$1 extends Error {
		constructor(msg) {
			super(msg);
		}
	}
	...
	var removeProxies = (obj, visited) => {
		...
		if (obj.$isProxy) {
			...
			if (obj.$isProxy) // If still a proxy.  There should never be more than 1 level deep of proxies.
				throw new XElementError$1("Double wrapped proxy found.");
			//#ENDIF
		}
	...
	*/
		...
		unsubscribe_(path, callback) {
			...
			if (cpath in this.subs_) {
				...
				if (callback) {
					...
					//#IFDEV
					if (callbackIndex === -1)
						throw new XElementError$1('Bad index');
					//#ENDIF
	 ...
	 * @param path {string|string[]}
	 * @param callback {function(action:string, path:string[], value:string?)} */
	var watch$1 = (obj, path, callback) => {
		obj = removeProxy(obj);

		// Keep only one WatchProperties per watched object.
		var wp = watch$1.objects.get(obj);
		if (!wp)
			watch$1.objects.set(obj, wp = new WatchProperties(obj));
	...
	var unwatch = (obj, path, callback) => {
		...
		if (wp) {
			...
			if (!Object.keys(wp.subs_).length)
				watch$1.objects.delete(obj);
		}
	};

	watch$1.cleanup = () => watch$1.objects = new WeakMap();
	...
	var createEl = (html) => {
		...
		if (typeof html !== 'string')
			throw new XElementError$1('Html argument must be a string.');
		//#ENDIF
	...
	var bindings = {
		...
		attribs: (self, code, el, context) => {
			...
			for (let name in obj) {
				...
				for (let path of parseVars(attrExpr)) {
					...
					watch$1(root, pathFromRoot, setAttr);
					exports.elWatches.add(el, [self,  path, setAttr]);
				}
		...
		classes: (self, code, el, context) => {
			...
			for (let name in obj) {
				...
				for (let path of parseVars(classExpr)) {
					let [root, pathFromRoot] = getRootXElement(self, path);
					watch$1(root, pathFromRoot, updateClass);
					exports.elWatches.add(el, [root, pathFromRoot, updateClass]);
				}
		...
		text: (self, code, el, context) => {
			...
			for (let path of parseVars(code)) {
				let [root, pathFromRoot] = getRootXElement(self, path);
				watch$1(root, pathFromRoot, setText);
				exports.elWatches.add(el, [root, pathFromRoot, setText]);
			}
		...
		html: (self, code, el, context) => {
			...
			for (let path of parseVars(code)) {
				let [root, pathFromRoot] = getRootXElement(self, path);
				watch$1(root, pathFromRoot, setHtml);
				exports.elWatches.add(el, [root,  pathFromRoot, setHtml]);
			}
		...
		loop: (self, code, el, context) => {
			...
			var rebuildChildren = (action, path, value, oldVal, indirect) => {
				...
				if (!root.loopHtml_) {
					...
					if (root.children.length !== 1)
						throw new XElementError$1('x-loop="' + code + '" must have exactly one child html element.  ' +
				...
				//#IFDEV
				if (!root.loopHtml_)
					throw new XElementError$1('x-loop="' + code + '" rebuildChildren() called before bindEl().');
				//#ENDIF
				...
				for (let i=0; i<root.children.length; i++) {
					...
					if (child.index_ !== i) {
						...
						if (loopVar in localContext)
							throw new XElementError$1('Loop variable "' + loopVar + '" already used in outer loop.');
						if (indexVar && indexVar in localContext)
							throw new XElementError$1('Loop index variable "' + indexVar + '" already used in outer loop.');
						//#ENDIF
			...
			for (let path of parseVars(foreach)) {
				let [root, pathFromRoot] = getRootXElement(self, path);
				watch$1(root, pathFromRoot, rebuildChildren);
				exports.elWatches.add(el, [root, pathFromRoot, rebuildChildren]);
			}
		...
		prop: (self, code, el, context) => {
			...
			//#IFDEV
			if (!context)
				throw new XElementError$1();
			if (!Array.isArray(context))
				throw new XElementError$1();
			if (!(el instanceof XElement))
				throw new XElementError$1('The data-prop and x-prop attributes can only be used on XElements in ' + code);
			//#ENDIF
			...
			if (self !== el) {
				...
				for (let prop in obj) {
					...
					//#IFDEV
					if (prop === 'this')
						throw new XElementError$1('Cannot use data-propx or x-prop to bind to "this" as a destination in ' + code);
					//#ENDIF
		...
		sortable: (self, code, el, context) => {
			...
			if (loopCode) {
				...
				if (!isStandaloneVar(foreach))
					throw new XElementError$1("Cannot bind sortable to non-standalone loop variable.");
				//#ENDIF
		...
		val: (self, code, el, context) => {
			...
			if (paths.length === 1 && isStandaloneVar(code)) {
				...
				exports.elEvents.add(el, ['input', onInput, null, self]);
			}
			...
			for (let path of paths) {
				let [root, pathFromRoot] = getRootXElement(self, path);
				watch$1(root, pathFromRoot, setVal);
				exports.elWatches.add(el, [root, pathFromRoot, setVal]);
			}
		...
		visible: (self, code, el, context) => {
			...
			for (let path of parseVars(code)) {
				let [root, pathFromRoot] = getRootXElement(self, path);
				watch$1(root, pathFromRoot, setVisible);
				exports.elWatches.add(el, [root, pathFromRoot, setVisible]);
			}
	...
	*/
	...
	var getLoopCode_$1 = (el) => el.getAttribute && (el.getAttribute('x-loop') || el.getAttribute('data-loop'));
		...
		if (el.getAttribute) { // if not document fragment
			...
			for (let eventName of eventNames) {
				...
				if (originalEventAttrib) {
					...
					exports.elEvents.add(el, [eventName, callback, originalEventAttrib, xelement]);
		...
		if (el.attributes)
			for (let attr of el.attributes) {
				if (attr.name.startsWith('x-') || attr.name.startsWith('data-')) {
					...
					// New
					let watches = exports.elWatches.getAll(el);
					for (let w of watches)
						unwatch(...w);
					exports.elWatches.removeAll(el);
				}
			}

		// Unbind events
		let ee = exports.elEvents.getAll(el) || [];
	...
	exports.getRootXElement = getRootXElement;
	exports.getXParent = getXParent;
	exports.removeProxy = removeProxy;
	exports.unbindEl = unbindEl;
